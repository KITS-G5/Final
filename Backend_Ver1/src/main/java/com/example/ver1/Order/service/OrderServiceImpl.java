...
public class OrderServiceImpl implements OrderService {
    ...
    public int saveOrder(Order order) {
        ...
        if (card.isPresent() && bikes.isPresent()) {
            if (card.get().getCardNum().equals(order.getCard().getCardNum())
                    && card.get().getCardCcv().equals(order.getCard().getCardCcv())) {
    ...
    float calculateFee(Order order) {
        int rentHours = (int) Math
                .ceil((order.getRentingEndDate().getTime() - order.getRentingStartedDate().getTime()) / 3600000);
